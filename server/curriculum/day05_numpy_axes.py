...
CHALLENGE_1 = {
    ...
    "description": (
        ...
        "and zero-center each student's scores using broadcasting."
    ),
    "instructions": (
        ...
        "2. Compute `student_averages`: 1D array of shape `(N,)` representing each student's average across all exams.\n"
        "3. Compute `exam_means`: 1D array of shape `(M,)` representing the class average for each exam.\n"
        "4. Compute `exam_stds`: 1D array of shape `(M,)` representing the standard deviation for each exam.\n"
        "5. Identify `top_student_idx`: `int` index of the student with the highest average score.\n"
        "6. Identify `hardest_exam_idx`: `int` index of the exam with the lowest average score.\n"
        "7. Compute `mean_centered_scores`: 2D array of shape `(N, M)` where each student's personal mean is subtracted from their exam scores, preserving dimensions during the intermediate average calculation for clean broadcasting.\n"
...
CHALLENGE_2 = {
    ...
    "instructions": (
        ...
        "2. Compute `running_balance`: 1D array representing the running cumulative balance over time.\n"
        "3. Compute `net_cashflow`: float representing total cash moved.\n"
        "4. Compute `mean_transaction`: float average of individual transactions.\n"
        "5. Compute `std_transaction`: float standard deviation of transactions.\n"
        "6. Identify `min_balance_idx`: int index where `running_balance` reached its minimum value.\n"
        "7. Identify `max_balance_idx`: int index where `running_balance` reached its peak value.\n"
        "8. Compute `outlier_mask`: 1D boolean array where `True` indicates $|\\text{tx} - \\text{mean}| > \\text{threshold\\_std} \\times \\text{std}$.\n"
        "9. Compute `outlier_indices`: 1D integer array containing the index locations of the outliers.\n"
