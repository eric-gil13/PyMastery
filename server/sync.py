"""Database persistence layer and multi-device sync manager for PyMastery with user accounts.

Supports SQLite (local default) and PostgreSQL (centralized cloud/team database)
adapters, automatically selected via the DATABASE_URL environment variable.
"""
...
def _load_env() -> None:
    """Load environment variables from .env file in root directory if present."""
...
def normalize_database_url(url: Optional[str]) -> Optional[str]:
    """Normalize database connection URL, replacing 'postgres://' with 'postgresql://'."""
    if not url:
        return None
    url = url.strip()
    if not url:
...
class BaseDatabaseAdapter:
    """Base interface and shared logic for PyMastery database adapters."""
    ...
    def get_progress(self, challenge_id: str, user_id: str = "default_user") -> Optional[ChallengeProgress]:
...
class PostgresDatabaseAdapter(BaseDatabaseAdapter):
    """PostgreSQL storage adapter for multi-device sync, shared auth, and progress."""
    ...
    def _get_connection(self):
        try:
            ...
            raise RuntimeError("psycopg is required for PostgreSQL support. Run `pip install psycopg[binary]`.") from e
    ...
    def register_user(self, username: str, password: str) -> dict:
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT id FROM accounts WHERE username = %s", (clean_user,))
                if cursor.fetchone():
                    return {"success": False, "message": "Username already taken"}
                cursor.execute("""
                    INSERT INTO accounts (id, username, password_hash, salt, created_at, last_login_at)
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (user_id, clean_user, pwd_hash, salt, now, now))
                cursor.execute("""
                    INSERT INTO user_profile (user_id, username, xp, level, streak_days, last_active_at, updated_at)
                    VALUES (%s, %s, 0, 1, 1, %s, %s)
                    ON CONFLICT(user_id) DO NOTHING
                """, (user_id, clean_user, now, now))
        token = generate_token(user_id, clean_user)
        return {"success": True, "user_id": user_id, "username": clean_user, "token": token, "message": "Account created successfully"}

    def login_user(self, username: str, password: str) -> dict:
        clean_user = username.strip()
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT * FROM accounts WHERE username = %s", (clean_user,))
                row = cursor.fetchone()
                if not row:
                    return {"success": False, "message": "Invalid username or password"}
                user_id = row["id"]
                ...
                if not verify_password(password, stored_hash, salt):
                    return {"success": False, "message": "Invalid username or password"}
                now = datetime.datetime.now(datetime.timezone.utc).isoformat()
                cursor.execute("UPDATE accounts SET last_login_at = %s WHERE id = %s", (now, user_id))
                token = generate_token(user_id, actual_username)
                return {"success": True, "user_id": user_id, "username": actual_username, "token": token, "message": f"Logged in as {actual_username}"}
    ...
    def get_full_user_state(self, user_id: str) -> dict:
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                prof = cursor.fetchone()
                profile_data = dict(prof) if prof else {"user_id": user_id, "username": "Student", "xp": 0, "level": 1}
                cursor.execute("SELECT * FROM challenge_progress WHERE user_id = %s", (user_id,))
                progress_rows = [dict(r) for r in cursor.fetchall()]
                cursor.execute("SELECT * FROM code_drafts WHERE user_id = %s", (user_id,))
                drafts_rows = [dict(r) for r in cursor.fetchall()]
                return {
                    "profile": profile_data,
                    "progress": {r["challenge_id"]: r for r in progress_rows},
                ...
                if challenge_id and code is not None:
                    ...
                    cursor.execute("""
                        ...
                        ON CONFLICT(user_id, challenge_id) DO UPDATE SET
                            ...
                            saved_at = EXCLUDED.saved_at
                    """, (record_id, user_id, challenge_id, code, now))
                cursor.execute("UPDATE user_profile SET last_active_at = %s, updated_at = %s WHERE user_id = %s", (now, now, user_id))
        return {"success": True, "saved_at": now}
    ...
    def get_draft(self, challenge_id: str, user_id: str = "default_user") -> Optional[CodeDraft]:
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT * FROM code_drafts WHERE user_id = %s AND challenge_id = %s", (user_id, challenge_id))
                row = cursor.fetchone()
                if not row:
                    return None
                return CodeDraft(
    ...
    def update_challenge_progress(
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT * FROM challenge_progress WHERE user_id = %s AND challenge_id = %s", (user_id, challenge_id))
                existing = cursor.fetchone()
                if existing:
                    status = "completed" if passed or existing["status"] == "completed" else "attempted"
                    attempts = existing["attempts_count"] + 1
                    ...
                    if memory_kb is not None:
                        best_memory = min(best_memory, memory_kb) if best_memory else memory_kb
                    best_code = code if (passed and not existing["best_code"]) else (existing["best_code"] or code)
                    new_passed_tests = max(existing["passed_tests_count"], passed_tests)
                    cursor.execute("""
                        UPDATE challenge_progress SET
                            status = %s,
                            best_code = %s,
                            attempts_count = %s,
                            passed_tests_count = %s,
                            total_tests_count = %s,
                            best_runtime_ms = %s,
                            best_memory_kb = %s,
                            first_completed_at = %s,
                            last_attempt_at = %s
                        WHERE id = %s
                    """, (status, best_code, attempts, new_passed_tests, total_tests, best_runtime, best_memory, first_completed, now, record_id))
                else:
                    status = "completed" if passed else "in_progress"
                    first_completed = now if passed else None
                    cursor.execute("""
    ...
    def _add_xp(self, user_id: str, amount: int, cursor=None):
        if cursor is not None:
            cursor.execute("SELECT xp FROM user_profile WHERE user_id = %s", (user_id,))
            row = cursor.fetchone()
            current_xp = row["xp"] if row else 0
            new_xp = current_xp + amount
            new_level = max(1, 1 + int(new_xp // 200))
    ...
    def get_highscores(self, challenge_id: str, limit: int = 10) -> List[BenchmarkHighscore]:
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT * FROM benchmark_highscores WHERE challenge_id = %s ORDER BY runtime_ms ASC LIMIT %s", (challenge_id, limit))
                return [
                    BenchmarkHighscore(
                        id=r["id"],
                        challenge_id=r["challenge_id"],
    ...
    def export_all(self, user_id: str = "default_user") -> SyncExportPayload:
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute("SELECT * FROM benchmark_highscores WHERE user_id = %s", (user_id,))
                benchmarks = [
                    BenchmarkHighscore(
                        id=r["id"],
                        challenge_id=r["challenge_id"],
...
def create_database_manager(db_url: Optional[str] = None) -> BaseDatabaseAdapter:
    """Factory to create PostgreSQL or SQLite database adapter based on DATABASE_URL."""
    _load_env()
    url = normalize_database_url(db_url or os.environ.get("DATABASE_URL"))
    if url and (url.startswith("postgresql://") or url.startswith("postgres://")):
        try:
            adapter = PostgresDatabaseAdapter(url)
            logger.info("Connected to centralized PostgreSQL database.")
            return adapter
        except Exception as exc:
            logger.warning("Failed to initialize PostgreSQL adapter (%s). Falling back to SQLite.", exc)
    return SQLiteDatabaseAdapter()


# Backward-compatible alias for existing imports and test suites
DatabaseManager = SQLiteDatabaseAdapter
